...
export default function GeneratePage() {
  ...
  useEffect(() => {
    const fetchTokenBalance = async () => {
      ...
      if (storedUser) {
        ...
        try {
          const response = await fetch(`${process.env.BACKEND_URL}/api/tokens/balance/${userData.id}`);
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const balanceResponse = await fetch(`${process.env.BACKEND_URL}/api/tokens/balance/${userData.id}`);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch(`${process.env.BACKEND_URL}/api/generate-complete`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
